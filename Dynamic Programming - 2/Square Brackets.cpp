...
/*
//need correction
int helper(int open, int close, int n, int k, int *a, bool *compulsion, int **dp)
{
    if(open > n || close > n)
        return 0;
    if(open == n && close == n)
        return 1;
    if(open == close || compulsion[open + close])
        dp[open][close] = helper(open + 1, close, n, k, a, compulsion, dp);
    else if(open == n)
        dp[open][close] = helper(open, close + 1, n, k, a, compulsion, dp);
    else
        dp[open][close] = helper(open + 1, close, n, k, a, compulsion, dp) + helper(open, close + 1, n, k, a, compulsion, dp);
...
int solve(int n, int k, int *a)
{
    ...
    bool *compulsion = new bool*[2*n];
    for(int i = 0; i < 2*n; i++)
    {
        compulsion[a[i]] = true;
    }
    
    int ans = helper(0, 0, n, k, a, compulsion, dp);
    for(int i = 0; i <= n; i++)
        delete [] dp[i];
    delete [] dp;
