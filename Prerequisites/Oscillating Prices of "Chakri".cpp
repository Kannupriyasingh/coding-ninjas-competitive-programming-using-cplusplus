...
int main() {
    ...
    while(i<n){
        if (a[i] < a[min_index]){
            //this means this element is smaller
            //therefore min_index needs to be of this index
            min_index = i;
        }
        int left_over = a[i] - a[min_index];
        if (left_over > max_amount){
            max_amount = left_over;
        }
        i++;
    }
